...
- **NEVER set, change, or infer branch tracking.** Always-banned commands regardless of user consent: `git push -u`, `git branch --set-upstream-to`, `git branch --track`, `git checkout --track`, `git switch --track`. Any other command that would write `branch.remote`/`branch.merge`: STOP and ask first.
- **Create local branches with tracking disabled.** Always use `git switch --no-track -c <branch>` (or `git checkout --no-track -b <branch>`).
- **`git worktree add` must use a local start-point** (commit SHA or local branch ref), never a remote ref or bare remote-tracking branch name. Keep tracking unset unless the user explicitly approves.
