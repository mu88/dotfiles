...
- **When launching background/sub-agents, pick the smallest model sufficient for the task's complexity** (e.g. simple/mechanical work → `claude-haiku-4.5` or `gpt-5-mini`; complex reasoning → keep default), unless the user explicitly names a model. Do not blindly inherit the current session's model.
