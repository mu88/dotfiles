...
- **Before creating a new branch, fetch the latest remote state and branch from it without tracking**: `git fetch origin <base-branch>` then `git switch --no-track -c <branch> origin/<base-branch>`. Never branch from a stale local ref or set tracking at creation time.
- **`git worktree add` must use a local start-point** (commit SHA or local branch ref), never a remote ref or bare remote-tracking branch name. Fetch first and resolve the SHA of the up-to-date base (`git fetch origin <base-branch>` then `git rev-parse origin/<base-branch>`) to use as start-point — this satisfies "local start-point" while guaranteeing freshness. Never set tracking at worktree creation.
- Before cloning a repo, **ALWAYS** check whether a local copy already exists inside or under the path assigned to the environment variable `LOCAL_DEV_DIR`. If it exists, use the existing copy and pull instead of cloning.
- **NEVER plain `git push`.** Every push:
  1. `$branch = git rev-parse --abbrev-ref HEAD` → if `HEAD` (detached), STOP and ask.
  2. `$upstream = git rev-parse --abbrev-ref --symbolic-full-name '@{u}' 2>$null` → if set and ≠ `origin/$branch`, STOP and ask (no auto-fix).
  3. `git push origin "${branch}:refs/heads/${branch}"`.
  4. If no upstream existed before the push, set it: `git branch --set-upstream-to="origin/${branch}" "${branch}"` — only when local/remote names match exactly; never track a feature branch to `origin/main` or `origin/master`.
- Permanently banned: `git push -u`, `git push --set-upstream`, `git branch --track`, `git checkout --track`, `git switch --track`.
...
- **When launching background/sub-agents, pick the smallest model sufficient for the task's complexity** (e.g. simple/mechanical work → the smallest/fastest tier of any available model family, such as Claude Haiku, GPT Mini, or Kimi; complex reasoning → keep default), unless the user explicitly names a model. Pick by family/tier, not a pinned version, so the choice doesn't go stale as versions change. Do not blindly inherit the current session's model.
- **During interactive sessions:** leave changes uncommitted for user review unless explicitly asked to commit.
- **Before starting a delegated multi-step task with expected low interaction** (multiple tool rounds, spans several files/modules, or phrased as an order rather than a discussion), **ask whether to isolate the work in a dedicated `feature/ai-*` worktree** to avoid race conditions with your concurrent manual work in the same repo. This only concerns branch/worktree isolation — commits still follow the interactive-session rule above unless you explicitly ask to commit.
- **On `feature/ai-*` branches (autonomous work):** always use a dedicated Git worktree for your work and clean it up (i.e. remove) after you're done. Commit with `--no-gpg-sign` (these branches are exempt from the signed-commits policy; signing will fail without a configured key). Amend/force-push commits when a change belongs to a previous commit semantically. Squash to clean, semantic commits before handing off for review. In autonomous (non-interactive) mode only — when invoked interactively on a feature/ai-* branch, the interactive-session rule above takes precedence: only the worktree/branch isolation applies, not the commit/squash/force-push ethic (see the worktree-isolation bullet above).
- Commit messages follow **Conventional Commits**. The body MUST EXPLAIN **why** — the diff shows what and how. Body: max 200 characters total, as a short paragraph or up to 3 bullet points; no file inventories. Never insert a line break to fit a column width (intentional deviation from the classic 72-char wrap rule, since commits here are read via GitHub/gh, which already soft-wraps) — write each paragraph or bullet as one unbroken line. If the "why" doesn't fit in 200 characters, ask rather than pad the message. After creating the commit, make sure that it contains an AI signature footer line (e.g. `Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>`).
...
- Never insert a line break to fit a column width (intentional deviation from the classic 72-char wrap rule, since GitHub already soft-wraps) — write each paragraph or bullet as one unbroken line. Always add an AI signature footer line to the PR description.
